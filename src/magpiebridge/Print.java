...
public class Print {
  ...
  public static CAstSourcePositionMap.Position getPosition(Statement s) {
  ...
  public static CAstSourcePositionMap.Position fixIncludedURL(CAstSourcePositionMap.Position p) {
    if (p instanceof IncludedPosition) {
      if (p.getURL().toString().matches(".*#[0-9]+$")) {
        class FlatPos extends AbstractSourcePosition {
          ...
          private final CAstSourcePositionMap.Position incp;

          private FlatPos(CAstSourcePositionMap.Position p) {
  ...
  public static String printStmt(Statement s) throws IOException {
    ...
    switch (s.getKind()) {
      case NORMAL:
        {
          if (ast) {
            CAstSourcePositionMap.Position p = getPosition(s);
            SourceBuffer buf = new SourceBuffer(p);
            return (buf + " (" + p + ")");
          } else {
            try {
              IMethod.SourcePosition p =
                  m.getSourcePosition(((NormalStatement) s).getInstructionIndex());
              if (p != null) {
                return p.toString();
              }
      ...
      case PARAM_CALLER:
        {
          if (ast) {
            ...
            try {
              IMethod.SourcePosition p =
                  m.getSourcePosition(((ParamCaller) s).getInstructionIndex());
              if (p != null) {
                return p.toString();
              }
            } catch (InvalidClassFileException e) {

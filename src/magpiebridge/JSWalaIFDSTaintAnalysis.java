...
public class JSWalaIFDSTaintAnalysis extends WalaIFDSTaintAnalysis {
  ...
  protected Function<BasicBlockInContext<IExplodedBasicBlock>, Boolean> sinkFinder() {
    return (bb) -> {
      ...
      if (inst instanceof SSAAbstractInvokeInstruction) {
        for (CGNode target :
            callGraph.getPossibleTargets(
                bb.getNode(), ((SSAAbstractInvokeInstruction) inst).getCallSite())) {
          if (target
              .getMethod()
              .getDeclaringClass()
              .getName()
              .toString()
              .contains("prototype_write")) {
            return true;
          }
        }
      }
  ...
  @Override
  public AstSSAPropagationCallGraphBuilder makeBuilder(
